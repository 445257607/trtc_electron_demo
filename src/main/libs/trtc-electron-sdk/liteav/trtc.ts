...
function formatLogParams(params: object): string {
  let logParamsStr = "";
...
class TRTCCloud extends EventEmitter {
  ...
  private id: number;
  private static isIPCMode: boolean | undefined;
  private static sharedTRTCCloudInstance: TRTCCloud | null;
  private static subInstances: TRTCCloud[];
  // private stateStore;
  private rtcCloud;
  ...
  constructor(config?: TRTCInitConfig) {
    let _a;
    super();
    this.id = !TRTCCloud.sharedTRTCCloudInstance
      ? 1
      : 2 +
        (((_a = TRTCCloud.subInstances) === null || _a === void 0
          ? void 0
          : _a.length) || 0);
    ...
    // this.remoteMainStreamMirrorType =
    ...
    // this.remoteSubStreamMirrorType =
    //   TRTCVideoMirrorType.TRTCVideoMirrorType_Disable;
   ...
   * @param {String} errMsg  - 错误信息。
   */
  handleOnSwitchRoom(errCode, errMsg): void {
   ...
   * @param {String} errMsg  - 错误信息。
   */
  handleOnSwitchRole(errCode, errMsg): void {
   ...
   * @param {String} errMsg  - 错误信息。
   */
  handleOnConnectOtherRoom(userId, errCode, errMsg): void {
   ...
   * @param {String} errMsg  - 错误信息。
   */
  handleOnDisconnectOtherRoom(errCode, errMsg): void {
   ...
   */
  handleOnUserAudioAvailable(userId, available): void {
    this.logger.log(
   ...
   */
  handleOnFirstAudioFrame(userId): void {
   ...
   */
  handleOnNetworkQuality(localQuality, remoteQuality): void {
   ...
   */
  handleOnStatistics(statis): void {
   ...
   */
  handleOnSpeedTestResult(result): void {
   ...
   */
  handleOnUserVoiceVolume(userVolumes, userVolumesCount, totalVolume): void {
   ...
   */
  handleOnDeviceChange(deviceId, type, state): void {
   ...
   */
  handleOnTestMicVolume(volume): void {
   ...
   * @param {Number} volume - 音量值，取值范围0 - 100
   */
  handleOnTestSpeakerVolume(volume): void {
   ...
   */
  handleOnAudioDeviceCaptureVolumeChanged(volume, muted): void {
   ...
   */
  handleOnAudioDevicePlayoutVolumeChanged(volume, muted): void {
   ...
   */
  handleOnRecvCustomCmdMsg(userId, cmdId, seq, msg): void {
   ...
   */
  handleOnMissCustomCmdMsg(userId, cmdId, errCode, missed): void {
   ...
   */
  handleOnRecvSEIMsg(userId, message): void {
   ...
   * @deprecated
   */
  handleOnStartPublishing(errCode, errMsg): void {
   ...
   * @deprecated
   */
  handleOnStopPublishing(errCode, errMsg): void {
   ...
   * @deprecated
   */
  handleOnStartPublishCDNStream(errCode, errMsg): void {
   ...
   * @deprecated
   */
  handleOnStopPublishCDNStream(errCode, errMsg): void {
   ...
   * @deprecated
   */
  handleOnSetMixTranscodingConfig(errCode, errMsg): void {
   ...
   */
  handleOnStartPublishMediaStream(taskId, code, message): void {
   ...
   * @param message {String} - 具体回调信息。
   */
  handleOnUpdatePublishMediaStream(taskId, code, message): void {
   ...
   * @param message {String} - 具体回调信息。
   */
  handleOnStopPublishMediaStream(taskId, code, message): void {
   ...
   */
  handleOnCdnStreamStateChanged(cdnUrl, status, code, msg): void {
   ...
   */
  handleOnAudioEffectFinished(effectId, code): void {
   ...
   */
  handleOnLocalRecordBegin(errCode, storagePath): void {
   ...
   * @param {string} storagePath - 录制文件存储路径
   */
  handleOnLocalRecording(duration, storagePath): void {
    let newDuration = 0;
    try {
   ...
   * @param {string} storagePath - 录制文件存储路径
   */
  handleOnLocalRecordComplete(errCode, storagePath): void {
  ...
  exitRoom(): void {
    ...
    //   this._destroyRenderer(key, null);
    // });
    // this.remoteVideoBufferMap.forEach((value, key) => {
    //   if (value.big) {
    //     this.rtcCloud.setRemoteVideoBuffer(
    //       key,
    //       value.big.buffer,
    //       value.big.streamType,
    //       0,
    //       0,
    //       value.big.pixelFormat,
    //       value.big.id,
    //     );
    ...
    //       key,
    //       value.small.buffer,
    //       value.small.streamType,
    //       0,
    //       0,
    //       value.small.pixelFormat,
    //       value.small.id,
    //     );
    ...
    //       key,
    //       value.sub.buffer,
    //       value.sub.streamType,
    //       0,
    //       0,
    //       value.sub.pixelFormat,
    //       value.sub.id,
    //     );
    ...
    //     "",
    //     this.localVideoBufferMap.big.buffer,
    //     this.localVideoBufferMap.big.streamType,
    //     0,
    //     0,
    //     this.localVideoBufferMap.big.pixelFormat,
    //     this.localVideoBufferMap.big.id,
    //   );
    ...
    //   this.rtcCloud.setLocalVideoBuffer(
    //     "",
    //     this.localVideoBufferMap.sub.buffer,
    //     this.localVideoBufferMap.sub.streamType,
    //     0,
    //     0,
    //     this.localVideoBufferMap.sub.pixelFormat,
    //     this.localVideoBufferMap.sub.id,
    //   );
    ...
    //     this.localVideoBufferMap.cameraTest.buffer,
    //     0,
    //     0,
    //     this.localVideoBufferMap.cameraTest.pixelFormat,
    //     this.localVideoBufferMap.cameraTest.id,
    //   );
   ...
   *
   */
  setDefaultStreamRecvMode(autoRecvAudio, autoRecvVideo): void {
